...
Then('Verify response details for tool {string}', (toolName) => {
  cy.get('@get_tools_data').then((response) => {
...
Then('Verify order status is {string}', (orderStatus) => {
  cy.get('@response').should((res) => {
...
Then('Write response in a response file named {string}', (fileName) => {
...
Then('The file with the accessToken exist {string}', (fileName) => {
    var filePath = "files/" + fileName + ".txt";
    cy.readFile(filePath).should("exist").and("contain", "accessToken");
});

Then('Create an order with clientName {string} and toolId {string} with the token in file {string}', (clientName, tooId, fileName) => {
  var filePath = "files/" + fileName + ".txt";
